...
function isPrimitive(value) {
  return typeof value in ['string', 'number', 'symbol', 'boolean'];
}
...
function updateListeners(on, oldOn, add, remove$$1, createOnceHandler, vm) {
  let name; let cur; let old; let event;
  for (name in on) {
    old = oldOn[name];
...
class FunctionalRenderContext {
  constructor(data, props, children, parent, Ctor) {
...
function createPatchFunction(backend) {
  ...
  function removeVnodes(_parentElm, vnodes, startIdx, endIdx) {
    for (; startIdx <= endIdx; ++startIdx) {
...
function updateAttrs(oldVnode, vnode) {
  ...
  /* istanbul ignore if */
  if ((isEdge) && attrs.value !== oldAttrs.value) {
...
function parseFilters(exp) {
  ...
  let c; let prev; let i; let expression; let filters;
...
let len; let str; let chr; let index$1; let expressionPos; let expressionEndPos;
...
const show = {
  ...
  unbind(el, _binding, _vnode, _oldVnode, isDestroy) {
    if (!isDestroy) {
      el.style.display = el.__vOriginalDisplay;
    }
  },
...
function parseText(text, delimiters) {
  ...
  let match; let index; let tokenValue;
...
function parseHTML(html, options) {
  ...
  while (html) {
    ...
    if (!lastTag || !isPlainTextElement(lastTag)) {
      ...
      const rest = html.replace(reStackedTag, function(_all, text, endTag) {
...
function processAttrs(el) {
  const list = el.attrsList;
  let i; let l; let name; let rawName; let value; let modifiers; let syncGen; let isDynamic;
...
const modules$1 = [klass$1, style$1, model$1];
...
function genDirectives(el, state) {
  ...
  let i; let l; let dir; let needRuntime;
